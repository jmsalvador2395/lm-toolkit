...
class AllSides(Dataset):
    ...
    def collate_fn(self, batch: List[Tuple]):
        ...
        for i, seq in enumerate(seqs):
            comb_indices = torch.tensor(list(
                combinations(range(lengths[i]), 2)
            ))
            breakpoint()
        """
        pair_lengths = torch.tensor(
            [comb(l, 2) for l in lengths],
            device=self.dev
        )
        N, S = len(lengths), max(pair_lengths)
        pair_indices = torch.tensor(
            list(combinations(range(max(lengths)), 2)),
            device=self.dev
        )
        ...
        pair_seqs = seqs[:, pair_indices]
        pair_seqs = pair_seqs.reshape((N, S, -1))
        ...
        # create sequence mask for pairs
        pair_seq_mask = torch.arange(max(pair_lengths)).repeat((N, 1))
        pair_seq_mask = pair_seq_mask.to(self.dev)

        pair_seq_mask = pair_seq_mask < pair_lengths[:, None]
        breakpoint()
        ...
        return {
            ...
            'sequence': pair_seqs.to(self.out_dev),
            'pair_indices': pair_indices.to(self.out_dev),
        }
