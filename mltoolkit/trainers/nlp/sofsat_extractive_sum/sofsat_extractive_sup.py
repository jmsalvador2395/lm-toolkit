...
# models
from mltoolkit.models import SofsatExtractor
...
class TrainerSofsatExtractiveSup(TrainerBase):
    def __init__(self, config_path, debug=False):
        super().__init__(config_path, debug=debug)

    def init_model(self):

        ########### assign values from cfg ########### 
        cfg = self.cfg

        mlp_hidden_size = cfg.model['mlp_hidden_size']
        hidden_layers = cfg.model['mlp_hidden_layers']
        out_size = cfg.model['out_size']

        ##############################################

        # define model
        dims = [768*3] + [mlp_hidden_size]*hidden_layers + [out_size]
    ...
    def step(self, model, batch, mode='train'):
        ...
        # make predictions
        scores = model(seq).squeeze()
        scores[~seq_mask] = float('-inf')
        ...
        labels = self.reward_model.get_scores(batch)
        breakpoint()
