...
class RewardModel(nn.Module):
    ...
    def build_mask(self, preds, seq_mask, k):
        ...
        out_idx = torch.zeros(N, dtype=torch.int32, device=dev)
        ...
        for i in range(S):
            ...
            # process for ones case
            mask = ones & (out_idx < k) & in_seq
            out_ids[mask, out_idx[mask]] = i
            out_idx[mask] += 1

            # process for twos case
            mask = twos & in_seq
            out_ids[mask, out_idx[mask]-1] = i

            # process for threes
            mask = threes & in_seq
            out_ids[mask, out_idx[mask]-2] = i
            out_idx[mask] = F.relu(out_idx[mask] - 1)
