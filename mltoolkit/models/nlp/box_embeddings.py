...
class BoxEmbeddings(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # gumbel parameters
        self.beta = cfg.get('beta', 1.9678289474987882)
        self.volume_temp = cfg.get('volume_temp', 0.33243242379830407)
        self.euler_gamma = 0.57721566490153286060

        # set model device
        self.dev = cfg.get('device', 'cpu')

        self.embedding_dim = cfg.get('embedding_dim', 200)
...
class Word2Box(BoxEmbeddings):
    ...
    def __init__(self, cfg):
        super(Word2Box, self).__init__(cfg)
